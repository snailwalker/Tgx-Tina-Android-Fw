...
public class DownloadInfo
{
	...
	public void sendIntentIfRequested(Uri contentUri, Context context) {
		...
		{
			Intent intent = new Intent(GlobalDownload.ACTION_DOWNLOAD_COMPLETED);
			intent.setClassName(mPackage, mClass);
			if (mExtras != null)
			{
				intent.putExtra(GlobalDownload.COLUMN_NOTIFICATION_EXTRAS, mExtras);
			}
	...
	public long restartTime() {
		if (mRetryAfter > 0) { return mLastMod + mRetryAfter; }
		return mLastMod +
	...
	public boolean isReadyToStart(long now) {
		if (mControl == GlobalDownload.CONTROL_PAUSED)
		{
	...
	public boolean hasCompletionNotification() {
		if (!GlobalDownload.isStatusCompleted(mStatus)) { return false; }
		if (mVisibility == GlobalDownload.VISIBILITY_VISIBLE_NOTIFY_COMPLETED) { return true; }
		return false;
	}
