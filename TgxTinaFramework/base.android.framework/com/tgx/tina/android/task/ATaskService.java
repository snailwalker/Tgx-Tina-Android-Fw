 ...
 * the License.
 *******************************************************************************/
package com.tgx.tina.android.task;
        ...
        TaskService
{
	...
	protected void onProcessorStop() {
		...
		for (;;)
		{
			boolean start = started.get();
			if (!start || started.compareAndSet(true, false)) break;
		}
	}
	...
	public final void startAService(Context context) {
		...
		{
			boolean start = started.get();
			if (start || started.compareAndSet(false, true)) break;
		}
	}
