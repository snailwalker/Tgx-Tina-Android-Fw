        ...
        ITaskFactory<String, IoSession<?>, IoFilter, IoTask<?>>
{
	...
	public IoTask<?> createTask(IoSession<?> ioSession, IoFilter ioFilter) {
		if (ioSession == null || ioFilter == null) return null;
	...
	public IoTask<?> createTask(IoSession<?> ioSession, int mode) {
		if (ioSession == null) return null;
		if (ioSession.status.equals(IoSession.Status.PREFETCHED) && ioSession.url.startsWith("socket") && mode == IConnection.CLOSE)
		{
			@SuppressWarnings ("unchecked")
