		...
		for (int i = 0; i < available && !disable;)
		{
			...
			switch (rawContactProfile.toDo) {
				...
				case RawContactProfile.toInsert:
					...
					{
						Builder builder = ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI).withValue(ContactsContract.Data.RAW_CONTACT_ID, rawContactId).withValue(ContactsContract.Data.MIMETYPE, ContactsContract.CommonDataKinds.StructuredName.CONTENT_ITEM_TYPE);
					...
					if (rawContactProfile.addresses != null)
					{
						for (String[] address : rawContactProfile.addresses)
						{
							...
							if (address[RawContactProfile.ADDRESS_NEIGHBORHOOD] != null) builder = builder.withValue(ContactsContract.CommonDataKinds.StructuredPostal.NEIGHBORHOOD, address[RawContactProfile.ADDRESS_NEIGHBORHOOD]);
					...
					if (rawContactProfile.orgs != null)
					{
						...
						{
							Builder builder = ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI).withValue(ContactsContract.Data.RAW_CONTACT_ID, rawContactId).withValue(ContactsContract.Data.MIMETYPE, ContactsContract.CommonDataKinds.Organization.CONTENT_ITEM_TYPE);
