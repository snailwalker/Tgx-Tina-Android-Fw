...
JNIEXPORT jintArray JNICALL Java_com_tgx_tina_android_plugin_contacts_search_SearchPy_initInfos(JNIEnv * env, jobject thiz, jobjectArray array, jint mode, jint oldAdr)
{
	...
	if (searchInfo == NULL) {
		searchInfo = (*env)->FindClass(env, "com/tgx/tina/android/plugin/contact/serach/SearchInfo");
	...
	for (i = 0; i < arrayLength; i++) {
		...
		if (str_name) {
			...
			(*env)->ReleaseStringChars(env, str_name, chars_name = (*env)->GetStringChars(env, str_name, (jboolean*) 0));
			(*env)->DeleteLocalRef(env, str_name);
		}

		if (str_phone) {
			phoneLength = (*env)->GetStringLength(env, str_phone);
			(*env)->ReleaseStringChars(env, str_phone, chars_phone = (*env)->GetStringChars(env, str_phone, (jboolean*) 0));
...
JNIEXPORT jintArray JNICALL Java_com_tgx_tina_android_plugin_contacts_search_SearchPy_search(JNIEnv * env, jobject thiz, jint adr, jstring key, jintArray lastSearched, jint start, jint limit, jlong filters, jboolean isT9)
{
	jintArray result = NULL;
	SearchTree* tree = NULL;
	int keyLength = 0;
	...
	if (lastSearched != NULL) {
		...
		if (lastSearchLength == 0 || start < 0) { //对数据边界进行校验.
			goto ToSearch;
		}
	...
	ToSearch: {
		...
		treeSearch(tree, keyChars, keyLength, toMalloc > 0 ? &(tree->lastSearched) : NULL
		, &(tree->desNameResult), &(tree->desPhoneResult), limit, filters, isT9);
...
JNIEXPORT jobjectArray JNICALL Java_com_tgx_tina_android_plugin_contacts_search_SearchPy_getHightLights(JNIEnv * env, jobject thiz, jint adr, jintArray searched, jstring keyText, jstring dyeStr, jboolean isT9)
{
